...
@pytest.mark.asyncio
async def test_qty_change(sample_product_inventory):
	assert sample_product_inventory.get_qty() == 0
	new_qty = 20
	sample_product_inventory.change_qty(qty=new_qty, change_type=ChangeType.RECEIVE)
	assert sample_product_inventory.get_qty() == pytest.approx(new_qty)
	reduce_by = 5
	sample_product_inventory.change_qty(qty=reduce_by, change_type=ChangeType.ISSUE)
	assert sample_product_inventory.get_qty() == pytest.approx(new_qty - reduce_by)
	new_qty = sample_product_inventory.get_qty()
	increase_by = 10
	sample_product_inventory.change_qty(qty=increase_by, change_type=ChangeType.RECEIVE)
	assert sample_product_inventory.get_qty() == pytest.approx(new_qty + increase_by)


@pytest.mark.asyncio
async def test_unit_conversion(sample_product_inventory):
	assert sample_product_inventory.get_qty(unit=Unit.BAG) == 0
	new_value_in_kg = 100
	new_value_in_bags = 2
	sample_product_inventory.change_qty(qty=new_value_in_bags, unit=Unit.BAG, change_type=ChangeType.RECEIVE)
	...
	reduce_by_bskts = 3
	sample_product_inventory.change_qty(qty=reduce_by_bskts, unit=Unit.BSKT, change_type=ChangeType.ISSUE)
...
@pytest.mark.asyncio
async def test_tiny_values(sample_product_inventory):
	assert sample_product_inventory.get_qty() == 0
	...
	sample_product_inventory.change_qty(qty=reduce_by_kg, unit=Unit.KG, change_type=ChangeType.ISSUE)
...
@pytest.mark.asyncio
async def test_qty_adjust(sample_product_inventory):
	assert sample_product_inventory.get_qty() == 0
...
@pytest.mark.asyncio
async def test_unsupported_stuff_unsupported(sample_product_inventory):
	assert sample_product_inventory.get_qty() == 0
...
@pytest.mark.asyncio
async def test_qty_limits(sample_product_inventory):	
	with pytest.raises(InvalidQtyError):
		sample_product_inventory.change_qty(qty=50, unit=Unit.BAG, change_type = ChangeType.ISSUE)

	with pytest.raises(InvalidQtyError):
		sample_product_inventory.change_qty(qty=-50, unit=Unit.BAG, change_type = ChangeType.RECEIVE)
